...
def Probe_Pulse(test_Pulse,drive_amp,num_rabi_points=50):
    drive_amp_min = -1
    drive_amp_max = 1
    drive_amps = np.linspace(drive_amp_min, drive_amp_max, num_rabi_points)#Hello
...
def Create_Pulse(inp):
    ...
    with pulse.build(backend=backend, default_alignment='sequential', name='Rabi Experiment') as custom_Pulse:
        [pulse.play(pulse.library.Constant(drive_duration, i), drive_chan)) for i in inp]
